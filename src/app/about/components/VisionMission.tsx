    ...
    >
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6">
        <div className={`text-center mb-12 transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          <span className="section-label text-accent/80 mb-3 block">Vision & Mission</span>
          <h2
            id="vision-mission-heading"
            className="font-display text-3xl sm:text-4xl text-primary-foreground"
          >
        ...
        <div className="grid sm:grid-cols-3 gap-6">
          {[
            ...
            <div
              key={item?.label}
              className={`bg-primary-foreground/8 border border-primary-foreground/15 rounded-3xl p-7 transition-all duration-700 hover:bg-primary-foreground/12 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
              style={{ transitionDelay: `${item?.delay}ms` }}
            >
              <div className="text-3xl mb-4" aria-hidden="true">{item?.icon}</div>
              <h3 className="font-display text-xl font-semibold text-primary-foreground mb-3">{item?.label}</h3>
