    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className={`mb-14 transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          <span className="section-label text-accent mb-3 block">Our Approach</span>
          <h2
            id="approach-heading"
            className="font-display text-3xl sm:text-4xl md:text-5xl text-foreground mb-5 max-w-2xl"
          >
            Building Solutions{' '}
        ...
        <div className="overflow-x-auto no-scrollbar pb-4">
          <div className="flex lg:grid lg:grid-cols-7 gap-4 min-w-max lg:min-w-0">
            {phases?.map((phase, i) => (
              <div
                key={phase?.num}
                className={`flex-shrink-0 w-48 lg:w-auto bg-background border border-border rounded-2xl p-5 hover:border-primary/40 hover:shadow-md transition-all duration-500 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}
                style={{ transitionDelay: `${i * 60}ms` }}
              >
                <div className="font-display font-bold text-3xl text-primary/20 mb-3">{phase?.num}</div>
                <h3 className="font-semibold text-foreground text-sm mb-2">{phase?.title}</h3>
