...
const navLinks = [
  ...
  { label: 'About', href: '/about' },
  {
    ...
    children: [
      ...
      { label: 'ART4Equity', href: '/programs/art4equity' },
            ...
            <div className="px-6 py-8 flex flex-col gap-2">
              {navLinks?.map((link) =>
                link?.children ? (
                  <div key={link?.label}>
                    <button
                      className="w-full flex items-center justify-between py-3 text-lg font-medium text-foreground border-b border-border focus-ring"
                      onClick={() => setProgramsOpen(!programsOpen)}
                      aria-expanded={programsOpen}
                    >
                      {link?.label}
                      <Icon name="ChevronDownIcon" size={18} className={`transition-transform ${programsOpen ? 'rotate-180' : ''}`} />
                    </button>
                    {programsOpen && (
                      <div className="pl-4 py-2 flex flex-col gap-1">
                        {link?.children?.map((child) => (
                          <Link
                            key={child?.href}
                            href={child?.href}
                            className="py-2.5 text-base text-foreground/70 hover:text-primary transition-colors focus-ring"
                  ...
                  <Link
                    key={link?.href}
                    href={link?.href}
                    className="py-3 text-lg font-medium text-foreground border-b border-border hover:text-primary transition-colors focus-ring"
                    onClick={() => setMenuOpen(false)}
                  >
                    {link?.label}
                  </Link>
              ...
              <div className="flex flex-col gap-3 pt-6">
                <Link
                  href="/donate"
                  className="w-full text-center bg-accent text-accent-foreground py-4 rounded-full font-semibold text-base focus-ring"
                  onClick={() => setMenuOpen(false)}
                >
                  Donate
                </Link>
                <Link
                  href="/get-involved"
                  className="w-full text-center bg-primary text-primary-foreground py-4 rounded-full font-semibold text-base focus-ring"
                  onClick={() => setMenuOpen(false)}
                >
                  Partner With Us
                </Link>
